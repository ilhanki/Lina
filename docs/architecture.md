...
`QtWindowsTTSProvider` mevcut PySide6 içindeki Windows WinRT motorunu yeni paket veya shell çalıştırmadan kullanır ve Türkçe WinRT sesini önceliklendirir. SAPI/System.Speech voice listesi WinRT listesine karıştırılmaz; yalnız SAPI tarafında bulunan sesler UI’da gösterilmez. Opsiyonel COM tabanlı `WindowsSapiTTSProvider` ayrı abstraction olarak korunur ancak runtime WinRT provider’a voice enjekte etmez. WinRT motoru bulunmazsa provider unavailable olur; timeline’daki yazılı cevap normal çalışır. TTS öncesi kod blokları, uzun URL, JSON/trace/Base64 benzeri içerik çıkarılır ve yalnız ses kopyası sınırlandırılır.
