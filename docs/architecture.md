...
`QtWindowsTTSProvider` mevcut PySide6 içindeki Windows WinRT/SAPI motorunu yeni paket veya shell çalıştırmadan kullanır ve Türkçe sistem sesini önceliklendirir. Opsiyonel COM tabanlı `WindowsSapiTTSProvider` abstraction içinde güvenli alternatif olarak korunur. Motor bulunmazsa provider unavailable olur; timeline’daki yazılı cevap normal çalışır. TTS öncesi kod blokları, uzun URL, JSON/trace/Base64 benzeri içerik çıkarılır ve yalnız ses kopyası sınırlandırılır.
