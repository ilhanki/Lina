...
Geniş pencerede sol navigation ve conversation workspace ana kabuğu oluşturur; sağ contextual inspector varsayılan olarak kapalıdır ve yalnız kullanıcı istediğinde üçüncü kolon olarak açılır. 900–1319 px aralığında sidebar açık kalır, inspector kullanıcı eylemiyle scrim üzerinde sağ drawer olur. 760–899 px aralığında sidebar ikon moduna geçer; composer ikincil araç etiketlerini gizler ve aynı drawer davranışını korur. Escape drawer’ı kapatır, odak inspector düğmesine döner; genişliğe bağlı otomatik kapanma kullanıcının kalıcı tercih değerini silmez.
...
Context inspector ana sayfası Voice, Vision ve File kısayollarını; güvenli Memory özetlerini ve gerçek local storage durumunu birleştirir. Agent ve Codex kısayolları yalnız ilgili gelişmiş özellik kullanıcı tarafından etkinleştirildiğinde görünür. Tool detayları aynı stacked yüzeyde progresif açılır. Memory repository yoksa veya boşsa bunun metinsel boş durumu gösterilir. Local storage hesaplaması async ve bounded’dır; hayali kota gösterilmez.
